...
def main() -> None:
    ...
    tracker = StageTracker(
        stage="clean_osm_city",
        city=city,
        run_id=args.run_id or generate_run_id(city),
        force=args.force,
    )
    command = f"python jobs/spark/clean_osm_city.py --city {city}"

    if tracker.should_skip(output_dirs):
        print(f"Skipping clean_osm_city for {city}; checkpoint exists for run_id={tracker.run_id}")
    ...
    try:
        ...
        roads_output_dir = clean_osm_dir(city, "roads")
        pois_output_dir = clean_osm_dir(city, "pois")
        roads_count = cleaned_roads.count()
        pois_count = cleaned_pois.count()
        cleaned_roads.write.mode("overwrite").parquet(str(roads_output_dir))
        cleaned_pois.write.mode("overwrite").parquet(str(pois_output_dir))
